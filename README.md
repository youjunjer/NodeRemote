...
ESP32/ESP8266/Pico W library for NodeAnywhere device claim + remote control.
...
搭配 NodeRemote 程式庫，你可以快速把 ESP32 / ESP8266 / Pico W 裝置接入 NodeAnywhere（https://node.mqttgo.io），將維運流程由「現場維護」轉成「雲端管理」。

教學影片：
- https://youtube.com/live/Grxjrlbm0Qs
...
- ESP8266 includes a built-in `Preferences` compatibility layer backed by `LittleFS`.
- Pico W includes an initial `LittleFS`-backed credential store and WiFi/MQTT support.
...
- Store device credentials in NVS / `LittleFS`-backed `Preferences`
...
```
Notes:
- Works for ESP32, ESP8266, and Pico W.
- Pico W support is the first step: claim / MQTT / serial / WiFi management are targeted first.
- In this version, Pico W does not yet support `sleep` or OTA flashing.
- The same TLS policy is applied to MQTT, HTTPS claim, and HTTPS OTA download on ESP32/ESP8266.
...
- Default commands:
  ...
  - `sleep 300` -> ACK then deep sleep (ESP32/ESP8266)
